...
if __name__ == "__main__":
    ...
    if not args.no_timestamp:
        args.savefile = f"{args.savefile}.time[{time()}]"
    ...
    if enforce:
        if train.shape[0] > test.shape[0]:
            train = train[
                np.random.choice(train.shape[0], test.shape[0], replace=False)
            ]
        elif test.shape[0] > train.shape[0]:
            test = test[np.random.choice(test.shape[0], train.shape[0], replace=False)]
    ...
    print(f"PRD in {time()-s}s")
    print("PRD:", prd)

    f_beta_data = prd_to_max_f_beta_pair(prd[0], prd[1], beta=8)  # Recall, Precision
